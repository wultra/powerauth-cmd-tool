...
The `qr-code-data` parameter is taken from QR code generated by PowerAuth RESTful services. Note that the QR code is signed, the signature is verified during offline signature computation. The `nonce` parameter is also available in the response for generating offline signature QR code. The method unlocks the knowledge related signing key using `1234` as a password.

The method does not execute any server calls due to its offline nature. The computed offline signature is used as an OTP and it is available from the output of the command in decimal format, e.g.: `"OFFLINE_SIGNATURE" : "99961544-80193814"`.
